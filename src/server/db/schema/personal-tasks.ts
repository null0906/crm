import { boolean, date, decimal, index, pgTable, text, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';
import { users } from './users';
import { projects } from './projects';
import { deals } from './deals';
import type { PersonalTaskStatus, TaskPriority } from '@/lib/types';
...
export const personalTasks = pgTable(
  'personal_tasks',
  {
    ...
    priority: varchar('priority', { length: 10 }).$type<TaskPriority>(),
  ...
  (t) => [
    ...
    index('idx_personal_tasks_assigned_by').on(t.assignedBy),
    ...
    index('idx_personal_tasks_due_date').on(t.dueDate),
