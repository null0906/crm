...
function requireExecutiveAccess(roleSlug?: string) {
...
export const executiveOverviewRouter = router({
  summary: protectedProcedure.query(async ({ ctx }) => {
    ...
    const activeProjects = (activeProjectsResult as { rows: Array<Record<string, unknown>> }).rows;
    const hotProspects = (hotProspectsResult as { rows: Array<Record<string, unknown>> }).rows;
    const completedProjects = (completedProjectsResult as { rows: Array<Record<string, unknown>> }).rows;
    const members = (membersResult as { rows: Array<Record<string, unknown>> }).rows;
      ...
      const [memberResult, prospectsResult, projectsResult, activitiesResult] = await Promise.all([
        ...
        db.execute(sql`
          SELECT DISTINCT
            ...
            ps.default_probability,
            CASE
              WHEN d.owner_id = ${input.userId} THEN 'Owner'
              WHEN d.created_by = ${input.userId} THEN 'Creator'
              ELSE 'Team member'
            END AS involvement
          FROM deals d
          JOIN pipeline_stages ps ON ps.id = d.stage_id
          ...
          WHERE d.deleted_at IS NULL
            AND d.status = 'open'
            AND (d.owner_id = ${input.userId} OR d.created_by = ${input.userId} OR dtm.user_id = ${input.userId})
        ...
        db.execute(sql`
          SELECT DISTINCT
            ...
            c.name AS company_name,
            CASE
              WHEN p.owner_id = ${input.userId} THEN 'Owner'
              WHEN p.created_by = ${input.userId} THEN 'Creator'
          ...
          LEFT JOIN project_members pm ON pm.project_id = p.id
          WHERE p.deleted_at IS NULL
            AND (p.owner_id = ${input.userId} OR p.created_by = ${input.userId} OR pm.user_id = ${input.userId})
          ORDER BY
        ...
        db.execute(sql`
          ...
          WHERE a.deleted_at IS NULL
            AND a.is_automated = false
            AND a.performed_by = ${input.userId}
          ORDER BY a.occurred_at DESC
          LIMIT 50
        `),
      ]);

      const member = (memberResult as { rows: Array<Record<string, unknown>> }).rows[0];
      if (!member) throw new TRPCError({ code: 'NOT_FOUND', message: 'Team member not found.' });

      return {
        member,
        prospects: (prospectsResult as { rows: Array<Record<string, unknown>> }).rows,
        projects: (projectsResult as { rows: Array<Record<string, unknown>> }).rows,
        activities: (activitiesResult as { rows: Array<Record<string, unknown>> }).rows,
      };
    }),
});
